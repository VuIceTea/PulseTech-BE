...
export class ProductService {
    buildProductListQuery(query = {}, options = {}) {
        const { page = 1, limit = 20, search, categoryId, brandId, minPrice, maxPrice, sort = "newest", isFeatured, isActive, } = query;
    ...
    async getProducts(query = {}) {
        const { skip, take, page, limit, where, orderBy } = this.buildProductListQuery(query, { includeInactive: false });
    ...
    async getAdminProducts(query = {}) {
        const { skip, take, page, limit, where, orderBy } = this.buildProductListQuery(query, { includeInactive: true });
        const [products, total] = await Promise.all([
            prisma.product.findMany({
                where,
                include: {
    ...
    async createVariant(productId, data) {
        ...
        // Validate stock
        const totalVariantStock = product.productVariants.reduce((sum, variant) => sum + variant.stock, 0);
        if (totalVariantStock + data.stock > product.stock) {
            throw new AppError("Tổng số lượng biến thể không được vượt quá tồn kho của sản phẩm ", 400);
        }
        // Check if SKU exists
