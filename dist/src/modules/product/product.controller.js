...
import { GetAdminProductsQueryDto, GetProductsQueryDto, } from "./dto/get-products-query.dto";
...
export const listAdminProductsHandler = async (req, res) => {
    try {
...
export const getFeaturedProductsHandler = async (req, res) => {
    try {
        const limitRaw = req.query.limit;
        const limit = typeof limitRaw === "string" && limitRaw.trim()
            ? Number(limitRaw)
            : undefined;
        const products = await productService.getFeaturedProducts(limit && Number.isFinite(limit) ? limit : undefined);
...
export const hardDeleteProductHandler = async (req, res) => {
    try {
        const productId = parseProductId(req.params.id);
...
export const createVariantHandler = async (req, res) => {
    try {
        const productId = parseProductId(req.params.id);
...
export const updateVariantHandler = async (req, res) => {
    try {
...
export const deleteVariantHandler = async (req, res) => {
    try {
        const variantId = parseProductId(req.params.variantId);
