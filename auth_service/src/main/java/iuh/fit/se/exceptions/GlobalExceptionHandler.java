...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiResponse<?>> methodArgumentNotValidExceptionHandler(
		...
		return ResponseEntity.badRequest().body(ApiResponse.builder().status("FAILED")
				.message("Registration Failed: Please provide valid data.").response(errorMessage).build());
	}
	...
	public ResponseEntity<ApiResponse<?>> userAlreadyExistsExceptionHandler(UserAlreadyExistsException exception) {
		return ResponseEntity.status(HttpStatus.CONFLICT)
				.body(ApiResponse.builder().status("FAILED").message(exception.getMessage()).build());
	}
}
