...
public class JwtTokenUtil {
	...
	public String generateToken(Authentication authentication, JwtEncoder jwtEncoder) {
		...
		try {
			...
			JwtClaimsSet claims = JwtClaimsSet.builder().issuer("iuh.fit.se").issuedAt(now)
					.expiresAt(generateExpirationDate()).subject(userPrincipal.getUsername())
					.claim("scope", userPrincipal.getAuthorities().stream().map(r -> r.getAuthority())
	...
	public boolean isTokenValid(Jwt jwtToken, UserPrincipal userPrincipal) {
		return !isTokenExpired(jwtToken) && userPrincipal.isEnabled()
