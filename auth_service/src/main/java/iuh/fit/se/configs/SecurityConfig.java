...
import iuh.fit.se.filters.JwtAccessTokenFilter;
import iuh.fit.se.services.TokenService;
import iuh.fit.se.services.impl.LogoutHandlerServiceImpl;
import iuh.fit.se.services.impl.UserDetailsServiceImpl;
...
public class SecurityConfig {
	...
	public SecurityConfig(RSAKeyRecord rsaKeyRecord, UserDetailsServiceImpl userDetailsService,
			JwtTokenUtil jwtTokenUil, LogoutHandlerServiceImpl logoutHandlerService, TokenService tokenService,
	...
	public SecurityFilterChain signUpSecurityFilterChain(HttpSecurity httpSecurity) throws Exception {
		httpSecurity.securityMatcher((new AntPathRequestMatcher("/sign-up"))).csrf(AbstractHttpConfigurer::disable)
	...
	public SecurityFilterChain signInSecurityFilterChain(HttpSecurity httpSecurity) throws Exception {
		httpSecurity.securityMatcher((new AntPathRequestMatcher("/sign-in"))).csrf(AbstractHttpConfigurer::disable)
				.authorizeHttpRequests(authorize -> authorize.anyRequest().permitAll())
	...
	SecurityFilterChain apiFilterChain(HttpSecurity httpSecurity) throws Exception {
		httpSecurity.securityMatcher((new AntPathRequestMatcher("/api/**")))
				...
				.addFilterBefore(new JwtAccessTokenFilter(this.jwtDecoder(), this.jwtTokenUil, this.userDetailsService,
						this.tokenService), UsernamePasswordAuthenticationFilter.class)
				...
				// However, as soon as any servlet-based configuration is provided, HTTP Basic
	...
	SecurityFilterChain logoutFilterChain(HttpSecurity httpSecurity) throws Exception {
		httpSecurity.securityMatcher((new AntPathRequestMatcher("/logout"))).csrf(AbstractHttpConfigurer::disable)
				.authorizeHttpRequests(authorize -> authorize.anyRequest().permitAll())
